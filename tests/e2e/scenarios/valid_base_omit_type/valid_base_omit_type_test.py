...
class TestValidBaseOmitTypeScenario:
	def test_should_successfully_build_the_project(self, tmp_path: pathlib.PosixPath):
		project_dir, result = project_build(Projects.VALID_BASE_OMIT_TYPE, tmp_path)

		assert result.returncode == 0
		assert pathlib.Path(project_dir / 'base-omit-type').exists()
